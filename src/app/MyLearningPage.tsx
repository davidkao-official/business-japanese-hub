...
function MyLearningContent({ snapshot }: { snapshot: PracticeLearningSnapshot }) {
  ...
  return <>
    ...
    {(snapshot.actionableMistakes.length > 0 || !empty) && <section className="my-learning-page__section" aria-labelledby="my-learning-review-title"><div><p className="my-learning-page__section-label">Review</p><h2 id="my-learning-review-title">最近答錯的題目</h2><p>稍後答對同一題後，它會依 #117 的 review 規則自然離開這裡。</p></div>{snapshot.actionableMistakes.length > 0 ? <ul className="my-learning-page__review-list">{snapshot.actionableMistakes.map((item) => { const action = actionFor(item, true); return <li key={`${item.contentId}:${item.questionId}`}><span>{categoryLabelFor(item)}</span>{action.href ? <Link to={action.href}>複習這一題</Link> : <span role="status">目前無法安全開啟</span>}</li> })}</ul> : <p>目前沒有可複習的錯題。</p>}</section>}
