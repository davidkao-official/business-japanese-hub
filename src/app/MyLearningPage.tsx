...
export function MyLearningPage({
  ...
  if (currentPageState.kind === 'unavailable') return <MyLearningShell><StatePanel title="學習紀錄暫時無法取得" body="目前無法讀取你的已儲存 evidence；不會用本機資料替代。" action={<button className="btn btn--secondary" type="button" onClick={() => setRequestKey((current) => current + 1)}>重試</button>} /><ReadingSavesSection key={user.id} ownerId={user.id} getAccessToken={getAccessToken} fetchSaves={fetchSaves} deleteSave={deleteSave} /></MyLearningShell>
