...
function MyLearningContent({ snapshot }: { snapshot: PracticeLearningSnapshot }) {
  ...
  return <>
    ...
    {(snapshot.actionableMistakes.length > 0 || !empty) && <section className="my-learning-page__section" aria-labelledby="my-learning-review-title"><div><p className="my-learning-page__section-label">Review</p><h2 id="my-learning-review-title">最近答錯的題目</h2><p>稍後答對同一題後，它會依 #117 的 review 規則自然離開這裡。</p></div>{snapshot.actionableMistakes.length > 0 ? <ul className="my-learning-page__review-list">{snapshot.actionableMistakes.map((item) => { const action = actionFor(item, true); return <li key={`${item.contentId}:${item.questionId}`}><span>{item.category}</span>{action.href ? <Link to={action.href}>複習這一題</Link> : <span role="status">目前無法安全開啟</span>}</li> })}</ul> : <p>目前沒有可複習的錯題。</p>}</section>}
    {!empty && <section className="my-learning-page__section" aria-labelledby="my-learning-recent-title"><p className="my-learning-page__section-label">Evidence</p><h2 id="my-learning-recent-title">最近的 Practice 作答</h2><ul className="my-learning-page__recent-list">{snapshot.recentAttempts.slice(0, 5).map((attempt) => <li key={`${attempt.contentId}:${attempt.questionId}:${attempt.createdAt}`}><span>{attempt.category}</span><span>{attempt.correct ? '回答正確' : '回答不正確'}</span><time dateTime={attempt.createdAt}>{new Date(attempt.createdAt).toLocaleDateString('zh-TW')}</time></li>)}</ul></section>}
    {!empty && <section className="my-learning-page__section" aria-labelledby="my-learning-signal-title"><p className="my-learning-page__section-label">分類訊號</p><h2 id="my-learning-signal-title">哪裡值得留意</h2>{snapshot.weakArea ? <p>{snapshot.weakArea.category}：最近 {snapshot.weakArea.sampleCount} 題中有 {snapshot.weakArea.incorrectCount} 題答錯，正答率 {snapshot.weakArea.accuracyPercent}%。</p> : <p>目前的作答樣本還不足以支持分類訊號。</p>}</section>}
  </>
}
...
function actionFor(item: PracticeReviewItem, review: boolean): { title: string; body: string; href: string | null; label: string } {
  ...
  return review
    ...
    : { title: '繼續 SPI 練習', body: `接著練習「${item.category}」；系統沒有保存單一 session 位置。`, href, label: '繼續這個類別' }
}
