...
const authoredScenario = {
  ...
  scenes: [
    ...
    },
    {
      ...
      context: '月曜日 16:00｜公開計画のフォローアップ',
      narrative:
        '公開計画レビューが終わり、懸念と選択肢を整理して武田部長に共有するフォローアップの時間になった。' +
        '品質保証の山田さんは、三件のうち一件は公開範囲を限定すれば回避でき、残り二件は追加検証が必要だと整理している。',
      dialogue: [
        {
          characterId: 'takeda-manager',
          text: '先ほどのレビューを踏まえ、二週間案を判断できるよう、必要な条件を具体的に整理してください。',
        },
        {
    ...
    },
    {
      ...
      narrative:
        '公開計画レビューを受け、公開範囲・日程・再判断の条件を資料に整理することになった。' +
    ...
    },
    {
      ...
      kind: 'decision',
      title: '決定後の実行条件を合意する',
      context: '金曜日 17:40｜公開計画のチーム確認',
      narrative:
        '公開計画の資料を受けて、次の方針をチームへ実行に移す段階になった。' +
        '資料上の確定事項と判断待ちの条件が曖昧なままだと、懸念を出した人が決定に従っていないようにも、無条件に約束したようにも見える。',
      dialogue: [
        {
          characterId: 'takeda-manager',
  ...
  outcomes: [
    ...
    },
    {
      ...
      category: 'strong',
      consequence:
        'チームは決定を共通認識にし、確認ポイント、リスクが再発した場合の報告先と時刻を合意して実行に移れた。',
      feedback:
        '異議を述べた後に決定へコミットすることは、懸念を撤回することではない。合意した条件を記録し、変化があれば早く報告できる仕組みまで作るのが上向きの提案の完成形。',
      recommendedExpression:
        '方針を承知しました。決定した内容と確認ポイント、リスクが再発した場合の報告先と時刻をチームに共有する、という認識で進めます。',
      acceptableAlternatives: [
