...
- **[`docs/content-model.md`](docs/content-model.md)** — 內容資料模型。`Book → Chapter → Content Blocks` 的具體定義與結構。
...
- **Supabase** 作為 server-authoritative auth / database / Edge Functions boundary
...
- **Canonical production frontend**：`https://business-japanese-hub.pages.dev/`（Cloudflare Pages）。GitHub Pages 不是 deployment target。
