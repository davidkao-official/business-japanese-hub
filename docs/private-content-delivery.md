...
`pnpm check:public-content-boundary` 是 CI guard：它比較 `books/`、`content-dist/books/` 與 legacy asset directories 對 allowlist，並驗證 Reader 的 static glob 沒有 private source/release store dependency。加入新的 static public Book 需要刻意更新 allowlist 並在 PR 說明其 non-proprietary/disclosed status；不能默默讓 #114/#125/#126 的 production corpus 走進 bundle。
