...
- Cloudflare Pages 使用 product-only validated build commands：Library 只驗證 released Books、Library TypeScript graph 與 `dist/`；Career Game 只驗證其 TypeScript graph 與 `dist-career-game/`。完整的 `pnpm build` 保留在 GitHub CI 作為同一 checkout 的 cross-product gate，因此一個產品的 build failure 不會阻塞另一個產品的獨立 deploy／rollback。
