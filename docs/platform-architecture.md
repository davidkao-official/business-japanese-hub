...
- Cloudflare Pages 使用 canonical product-only validated build commands `pnpm build:library:deploy` 與 `pnpm build:career-game:deploy`；Library 只驗證 released Books、Library TypeScript graph 與 `dist/`，Career Game 只驗證其 TypeScript graph 與 `dist-career-game/`，而 artifact-isolation tests 直接執行這兩個 commands。完整的 `pnpm build` 保留在 GitHub CI 作為同一 checkout 的 cross-product gate，因此一個產品的 build failure 不會阻塞另一個產品的獨立 deploy／rollback。
...
- Generic product switching uses the counterpart product root, so Library never hard-codes the Game default Scenario. Content-specific cross-links require an explicit stable-reference metadata seam owned by the source content.
