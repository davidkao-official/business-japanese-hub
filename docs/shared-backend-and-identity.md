...
| Future proprietary member delivery | authenticated `content-delivery` Edge Function only | immutable bounded-runtime release payload | browser has no table/RPC path; the Edge Function uses a service-role temporal membership RPC over server-owned paid windows, otherwise fails closed |
