...
- `PracticeStudyManifest` is frozen before study outcomes: it records the exact question-bank version, top-level checkpoint-registry version, one study-level `supportLocale`, and each item's registry version (which must equal the top-level version). It also records each selected support-overlay artifact as `(questionId, questionVersion, locale, overlayVersion)`, every matched block's four-item membership and transfer pairings, and a versioned participant-slot counterbalancing schedule whose trial rows pin the item-to-condition assignment plus block / trial order. Ordinary-explanation trials select no support overlay; foreigners-first trials may use only the exact matching overlay reference in that manifest. A later translation or allocation edit creates a new manifest and cannot silently change a completed session.
- The manifest validator must reject duplicate item / trial / block IDs, duplicate `participantSlot` entries, an `itemSet` question without a non-empty `diagnosticCheckpoints.ids` list, a matched-block item that does not resolve uniquely to one validated `itemSet` row, a trial whose question or transfer pair does not match its matched-block item, a transfer item pair that does not resolve uniquely in the pinned `PracticeQuestionBank.version`, a transfer pair that equals any original question in the same participant-slot schedule or is reused within that schedule, a support-overlay ref that does not match its question, the study-level `supportLocale` and one listed `selectedSupportOverlays` entry, an ordinary trial that carries a support overlay, or a foreigners-first trial without one whose exact `byLocale[locale]` payload has at least one non-empty support field. It must also reject a block that does not contain exactly four distinct items, duplicate original question/version pairs across the matched blocks covered by one participant schedule, a participant-slot schedule with fewer than two blocks or whose `blockOrder` is not an exact permutation of the blocks represented by its trials, a schedule that references unknown or repeated trials or fails to assign those items exactly once to each explanation × checkpoint-feedback combination, and any block whose trial `order` values are not an exact permutation of `1..4`. Across all scheduled participant slots, each block item must rotate through the four treatment cells equally, or as evenly as the slot count permits (cell counts may differ by at most one); at each within-block trial position, the four treatment cells must also occur equally or as evenly as the slot count permits. A fixed item-to-cell or fixed treatment-order permutation is invalid. Every `itemSet` row must resolve to a question whose `itemAnalysis.diagnosticCheckpoints.registryVersion` equals both that row's and the manifest's `checkpointRegistryVersion`; a mismatch is invalid rather than silently replayed.
...
- The Stage 1 validator must resolve every `diagnosticCheckpoints.ids` entry in that registry version, require checkpoint IDs to be unique within each immutable registry, and reject any checkpoint whose `questionId` or `questionVersion` does not match the containing `PracticeQuestion`.
...
- Stage 1 question-bank validation must require every `(question.id, question.version)` pair in `PracticeQuestionBank.questions` to be unique; duplicate identities are invalid because a manifest reference must resolve to exactly one prompt, answer and explanation.
...
- Stage 1 answer validation must require non-empty `choices` for multi-select and ordering inputs, unique choice IDs, a single-choice `choiceId` to be offered, multi-select `choiceIds` to be a unique subset of offered IDs, and ordering `choiceIds` to be an exact permutation of offered IDs. An intentionally empty multi-select answer needs an explicit option or separately defined semantics. These rules apply to both questions and checkpoints.
...
4. Before observing any outcomes, freeze a `PracticeStudyManifest` containing only study items with non-empty checkpoint lists, the exact question-bank version, top-level and per-item registry versions, the study-level support locale (`zh-Hant` for the first study), selected support-overlay artifact/version for the foreigners-first cells, matched-block item membership and transfer pairings, every participant-slot trial's item-to-condition assignment and transfer-item/version pairing, plus the `counterbalanceScheduleVersion` and block / trial order. Assign each matched item block four distinct comparable original questions: ordinary + feedback hidden, ordinary + feedback visible, foreigners-first + feedback hidden, and foreigners-first + feedback visible. Counterbalance item-to-cell assignment and condition order across participants by storing a unique per-slot condition permutation whose within-block `order` values are exactly `1..4`, rotates each item equally (or as evenly as the number of slots permits) through all four cells, and places each treatment cell equally (or as evenly as possible) at each within-block trial position; do not resolve an overlay from the latest catalog entry, mix support locales within one study, reuse an original question across matched blocks, or reconstruct allocation after a session.
...
7. Compare time-to-understanding and a short post-explanation transfer task for the ordinary vs foreigners-first conditions, using the feedback-hidden cells for the primary explanation comparison. Every transfer item must be newly authored, resolve in the frozen question bank, and be disjoint from every original question and every other transfer item in that participant slot. For checkpoint value, compare both alignment with the predeclared rule and transfer accuracy / time between feedback-visible and matched feedback-hidden cells under the same explanation.
