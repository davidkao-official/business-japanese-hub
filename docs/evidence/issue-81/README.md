...
## Historical anchors and current-candidate boundary
...
- Historical implementation/capture anchor: `bc0e016cfcb3d187b7fef8cc2bf8b30ce0066563`.
- Machine-readable historical capture and Lighthouse provenance: [`qa-manifest.json`](qa-manifest.json).
- These anchors and all measurements below are retained for historical provenance only. They are non-repeatable evidence from the recorded #81 source range, not current final-candidate evidence.
- The refresh candidate being repaired started from `origin/main` at `c6579bb754380c7e3aa4d0f277779509d164b36d` and pre-repair HEAD `6acb4743fa28a5cff3589c13b3bd1f4c02f0fe0d`; this README does not claim that the historical captures, Lighthouse results, or DB results were rerun at that candidate.
...
## Product and interaction QA (historical capture scope)

The following bullets summarize the historical capture artifact only; they are not current refresh-candidate assertions.
...
- Historical accessibility QA: Lighthouse accessibility 100 at both requested viewports; semantic nav/dialog/footer landmarks, headings, link/button names, focus-visible treatment, contrast, and the shared reduced-motion rule were active in that capture.
...
## Historical Lighthouse 13.4.1 (not current-candidate evidence)

The following values were recorded from the historical source range above using the exact requested viewport emulation, device scale factor 1, and the same local production-preview method as the baseline. They are not a current refresh-candidate run. The historical #78 / PR #88 authority was desktop 100 / 100 and mobile 96 / 100; the fresh post-#80 baseline was rerun because the current main artifact was longer than that historical surface.
...
- The exact pre-#80 and post-#80 reports identify the same existing featured Book cover as the mobile LCP. It is the released `cover-B0Kzu4AB.jpg` asset at 446,868 bytes; Lighthouse estimates 426,601 wasted bytes because the 1,086 × 1,448 source is oversized for the 302 × 403 mobile render and is not a modern format. The historical current-main and historical final reports also share the 150 ms render-blocking CSS, approximately 98 KiB unused JavaScript, and the same LCP-discovery limitation that the catalog-rendered image is not discoverable in the initial HTML.
- #81 applied the safe surface-local remediation available without changing the released Book asset or the #80 editorial content model: the above-the-fold featured cover uses `loading="eager" fetchPriority="high"`. The historical final report confirmed the priority/eager checks were true; the remaining request-discovery check was false because the catalog is resolved by the existing client runtime. Historical final and current-main sequential runs remained 83 / 100 with identical 4.4 s LCP and the same failing-audit set.
- The pre-#80 → post-#80 delta is therefore documented as pre-existing to #81, with the exact source commits, timestamps, and report digests in `qa-manifest.json`. The measured historical current-main/final audit set was the same, while #81's only above-the-fold image change was the eager/high hint. Removing the published #80 editorial projection or silently changing the released asset/pipeline would alter an upstream product/content boundary; a versioned asset-pipeline optimization is the concrete follow-up seam, rather than an unsafe scope expansion in this final LP PR.

## Historical validation record (not current candidate evidence)

The following results were recorded for the historical #81 source range ending at the historical implementation anchor above. They are historical/non-repeatable records, not repository-prescribed gates for the refresh candidate, and this README does not assert them at current HEAD:
...
- Historical raw DB commands (pre-#98 safety boundary; retained for provenance only):
...
These raw commands must not be rerun or treated as current repository-prescribed validation. Current DB validation is only `pnpm validate:db`, through the guarded owned disposable daemon described in `AGENTS.md` and `docs/db-validation.md`; no current-candidate DB PASS is claimed here.

CI and mergeability remain GitHub-authoritative. Any current-candidate screenshot, Lighthouse, DB, or exact-HEAD result must be generated at the exact candidate HEAD and recorded separately; this historical package intentionally does not supply that evidence.
