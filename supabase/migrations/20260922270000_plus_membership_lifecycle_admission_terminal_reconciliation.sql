-- #164 admission marker and terminal reconciliation (append-only repair of
-- 20260922260000).
--
-- This migration keeps every invariant and repair from 20260922260000 and adds
-- three bounded fixes.
--
...
-- membership_started is durable and retired C's binding while the prior stream
-- A stayed current (the non-current branch only ever retires its own binding).
-- The later confirmed C start was then rejected as stale because C was already
-- retired, so forward delivery (terminal before start) left A active while
-- reverse delivery (start before terminal) ended terminated. The fix keeps the
-- retirement but lets a stream retired by its own durable terminal evidence
-- still accept a lifecycle event that occurred strictly before that terminal.
...
-- current, so C is ordered against its own confirmed start and the buffered fold
-- below uses that C ordering instead of the stale A watermark.
--
-- P3 (plan admission): the active-plan gate treated any preexisting binding as
...
-- Preserve previously valid admissions. A stream that already durably recorded
-- a confirmed activation before this migration keeps accepting renewal,
-- reactivation and terminal evidence after a plan deactivation. Pending-only
-- and otherwise unconfirmed bindings stay unadmitted and are still gated.
update public.plus_membership_subscription bound
set admitted_at = coalesce(bound.admitted_at, bound.bound_at)
where bound.admitted_at is null
  and exists (
    select 1
    from public.plus_membership_event activation
    where activation.source_system = bound.source_system
...
begin
  ...
  if v_subscription_retired then
    ...
    -- lifecycle event that occurred strictly before that terminal. A distinct
    -- successor's terminal can be delivered before its own confirming start
    -- (the terminal retires only its own binding in the non-current branch
    -- above), and refusing the start here would leave the previously current
    -- stream active forever, diverging from the reverse delivery where the same
    -- start becomes current first and the terminal then ends it. The retirement
    -- itself is preserved and the event must still beat the current stream's
    -- ordering barrier below, so a superseded/replaced stream, an event at or
    -- after its own terminal, and an old terminal that must not mutate an
    -- unrelated current stream all keep returning stale.
  ...
  -- terminal authority. A cancellation is terminal evidence rather than an
  -- activation, so it is never gated.
  ...
  if v_stream_became_current then
    ...
    if found then
      if v_buffered_status in ('canceled', 'expired', 'revoked') then
        update public.plus_membership_subscription
        set retired_at = coalesce(retired_at, now()),
            retired_event_id = coalesce(retired_event_id, v_buffered_event_id)
  ...
  -- The activation actually applied, so the stream is durably admitted: later
  -- renewal/reactivation/terminal evidence on it stays acceptable even if the
  -- plan is deactivated afterwards. An unconfirmed pending event does not set
  -- the marker, so a later start/renewal on such a binding is still gated by
  -- plan availability. The marker is monotonic and never cleared, including
  -- when the applied event is immediately terminated by the fold above.
  if v_membership_status = 'active' and p_event_type <> 'membership_canceled' then
    update public.plus_membership_subscription
