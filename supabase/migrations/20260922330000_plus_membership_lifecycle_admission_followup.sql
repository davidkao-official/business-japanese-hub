...
-- by that exception, so the start may select/fold the retired stream.
  ...
  if v_stream_became_current then
    ...
    if found then
      -- This buffered start now becomes the authoritative active projection,
      -- so it must receive the identical active-plan admission treatment as a
      -- direct start. The event was durable evidence while stale, but it is not
      -- admission until this fold actually makes it authoritative.
      if v_buffered_event_type = 'membership_started'
         and v_buffered_status = 'active' then
        if not exists (
          select 1 from public.plus_membership_plan
