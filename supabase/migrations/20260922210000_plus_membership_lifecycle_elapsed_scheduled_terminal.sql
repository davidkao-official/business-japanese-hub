-- #164 effective earliest terminal authority as the cross-stream barrier.
--
-- 20260922190000 made a retired current stream's durable scheduled terminal_at
-- the cross-stream ordering barrier. That is only correct when the scheduled
-- cutoff is the stream's effective end. When an immediate terminal (a refund,
-- revoke or immediate cancellation) took effect before the scheduled period-end
-- cutoff, the stream actually ended at the immediate terminal, so ordering a
-- distinct successor against the later terminal_at wrongly rejects a legitimate
-- membership_started delivered between the immediate terminal and the cutoff.
--
-- The barrier for a retired current stream is therefore the effective earliest
-- terminal authority: the earliest immediate terminal evidence -- its
-- (occurred_at, event_id) -- when that is earlier than the durable scheduled
-- cutoff, otherwise the scheduled terminal (terminal_at, terminal_event_id).
-- Lock order, successor retirement, source-binding retirement, replay-fact
-- immutability, non-retired reducer ordering and durable future period-end
...
-- When the retired current stream also carries earlier immediate terminal
-- evidence, that immediate evidence -- not the later scheduled terminal_at -- is
-- the effective end that a successor must be strictly newer than. A successor
-- older than the barrier stays stale no matter when it is delivered, and a
-- retired stream can never resurrect because its own binding rejects every later
-- arrival before state is touched.
--
...
--
-- P3: a durable scheduled cutoff that wall-clock time has already reached ends
-- the current stream even when no later event has retired its binding yet. The
-- cross-stream path previously ordered a successor against the reducer clock
-- for any non-retired current stream, so a successor whose occurred_at fell
-- before the elapsed cutoff could take authority merely because it was
-- delivered after the cutoff. A current stream whose terminal_at <= now() is
-- now retired durably before the successor comparison, and the effective
-- earliest terminal authority (an earlier immediate terminal, otherwise
-- terminal_at / terminal_event_id) becomes the barrier: a pre-cutoff arrival
-- stays stale no matter when it is delivered, while a successor strictly after
-- the cutoff still applies. Lock order, immediate-vs-scheduled terminal
-- selection, noncurrent terminal evidence handling, source-subscription
-- retirement and no-resurrection rules are preserved.
