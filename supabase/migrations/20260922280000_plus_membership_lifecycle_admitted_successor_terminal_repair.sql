-- #164 admitted successor terminal repair (append-only repair of
-- 20260922260000).
--
-- This migration keeps every invariant and repair from 20260922260000 and adds
-- the bounded admission and successor-terminal fixes below.
--
...
-- current. The binding must remain unretired while C is unadmitted, otherwise
-- the later confirmed C start is rejected as stale and forward delivery
-- diverges from reverse delivery. Once C is selected, the fold retires it from
-- the same durable terminal evidence.
...
declare
  ...
  v_defer_terminal_retirement boolean := false;
...
begin
  ...
  -- Terminal facts on an unadmitted successor are durable audit evidence, but
  -- must not retire that binding while another stream is current: its earlier
  -- membership_started may still legitimately select it. An admitted stale
  -- stream, or a terminal stream with no current competitor, is retired here.
  select * into v_state
  from public.plus_membership_state
  where user_id = v_subscription.user_id
  for update;
  v_defer_terminal_retirement := (v_terminal or v_boundary_reached)
    and v_subscription.admitted_at is null
    and found
    and (v_state.source_system <> v_subscription.source_system
      or v_state.source_customer_id <> v_subscription.source_customer_id
      or v_state.source_subscription_id <> v_subscription.source_subscription_id);

  if (v_terminal or v_boundary_reached) and not v_defer_terminal_retirement then
    update public.plus_membership_subscription
  ...
  if v_subscription_retired then
    ...
    -- lifecycle event that occurred strictly before that terminal. This path
    -- remains for already-retired historical rows; newly unadmitted successor
    -- terminal evidence is kept live by v_defer_terminal_retirement above.
    -- The event must still beat the current stream's ordering barrier below,
  ...
  if found then
    ...
    if v_state.source_system = v_event.source_system
      ...
      if (v_terminal or v_period_end_terminal) and not v_defer_terminal_retirement then
        update public.plus_membership_subscription
        set retired_at = coalesce(retired_at, now()),
            retired_event_id = coalesce(retired_event_id, v_event.event_id)
        where source_system = v_subscription.source_system
