-- #164 terminal-evidence ordering for cross-stream successors.
--
...
-- authority, replay-fact immutability and all earlier behaviors are preserved:
...
-- a distinct non-retired stream must then still be able to become current, but
-- only when it is genuinely newer than the evidence that ended the retired
-- stream. Ordering is therefore not disabled: while the current stream stays
-- live its candidates are ordered against its reducer clock, and once the
-- current binding is retired they are ordered against the durable terminal
-- evidence (the stream's earliest terminal event). A successor older than that
-- evidence stays stale no matter when it is delivered, and a retired stream can
-- never resurrect because its own binding rejects every later arrival before
-- state is touched.
--
...
begin
  ...
  if found then
    if v_state.source_system = v_event.source_system
      ...
      -- current binding is ordered against the durable terminal evidence that
      -- ended it instead of the reducer clock, so a successor older than that
      -- evidence stays stale no matter when it is delivered. A non-retired
      -- current stream keeps strict (occurred_at,event_id) ordering, and a
      -- retired stream can never resurrect because its own binding is
