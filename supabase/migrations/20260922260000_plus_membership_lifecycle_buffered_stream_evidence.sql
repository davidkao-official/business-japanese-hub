-- #164 buffered successor-stream evidence (append-only repair of
-- 20260922250000).
--
-- This migration keeps every invariant and repair from 20260922250000 and adds
-- two bounded fixes.
--
-- P1 (convergence): a non-start event on a distinct successor stream that is
-- delivered before that stream's own confirmed membership_started is durable
-- and returned stale while another stream stays current (for example C
-- membership_payment_failed t15 before C membership_started t10 while pending
-- B is current). Once the confirmed C start displaces the live pending barrier,
-- C becomes active, but the earlier-recorded failure stays unapplied and the
-- durable row can never apply again because a replay returns 'replayed'. The
-- fix folds the durable active/past_due evidence already recorded for a stream
-- that just became current: after the applied event, the newest such evidence
-- strictly newer than the applied key and the monotonic watermark is adopted as
-- the current state/access. Forward delivery (failure before start) and reverse
-- delivery (start before failure) therefore converge on the same past_due
-- result, an unconfirmed pending is still outranked by the applied
-- confirmation, and the watermark only ever moves forward.
--
-- P2 (plan admission): the active-plan gate used to fire for any event that
-- mapped to active, including a period-end membership_canceled, so a stream
-- whose plan was later marked inactive could no longer record its own
-- cancellation: the gate raised and rolled back the durable terminal cutoff.
-- The gate now applies only to a genuinely new activation -- an event that
-- creates the source binding and would project active access, excluding
-- terminal/cancellation evidence -- so renewal and period-end cancellation on
-- an already-bound stream stay accepted.
--
...
-- resurrect, durable elapsed period-end cutoff as terminal authority, immediate
...
begin
  ...
  -- Plan availability gates only a genuinely new activation: an event that
  -- creates the source binding and would project active access. Evidence on an
  -- already-bound stream -- a renewal, a reactivation, or a period-end
  -- cancellation that maps to active -- must stay accepted, otherwise a later
  -- plan deactivation rolls back durable terminal authority. A cancellation is
  -- terminal evidence rather than an activation, so it is never gated.
  if not v_binding_preexisting
     and v_membership_status = 'active'
     and p_event_type <> 'membership_canceled'
     and not exists (
  ...
  -- non-start evidence that arrived before its own confirmation and was
  -- rejected as stale while another stream was current (for example a payment
  -- failure recorded before the confirming start). That evidence has been
  ...
  -- delivery (failure before start) stays active while reverse delivery (start
  -- before failure) ends past_due. Only active/past_due evidence is folded --
  -- an unconfirmed pending is outranked by the applied confirmation, and
  -- terminal/period-end evidence already retired its own binding before this
  -- point. The fold is bounded by the applied event key and the monotonic
  -- watermark, so it can never rewind ordering or resurrect a retired stream.
  if v_stream_became_current then
    select buffered.membership_status, buffered.plan_code,
           buffered.period_start, buffered.period_end,
           buffered.cancel_at_period_end,
           buffered.occurred_at, buffered.event_id
    into v_buffered_status, v_buffered_plan_code,
    ...
    where buffered.source_system = v_event.source_system
      ...
      and buffered.membership_status in ('active', 'past_due')
    ...
    order by buffered.occurred_at desc, buffered.event_id desc
    limit 1;
