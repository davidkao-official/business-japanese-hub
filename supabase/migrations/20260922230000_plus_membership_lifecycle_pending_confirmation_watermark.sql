-- #164 pending succession, same-stream confirmation precedence, and the
-- monotonic reducer watermark.
--
...
-- pending-state threads left by 20260922210000, plus one native P1 left by
-- 20260922220000.
--
...
--
-- P1c: the same-stream confirmation exemption above must take semantic
-- state/access precedence without lowering the reducer ordering watermark.
-- A pending event with a higher event id (for example source id zzz) can be
-- recorded first at occurred_at T; a confirming membership_started with a
-- lower event id (aaa) at the same T then activates the state. Because the
-- confirmation's event key is ordered below the recorded pending key, writing
-- the confirmation's own key into last_event_occurred_at/last_event_id rewinds
-- the watermark and lets a later same-timestamp event (payment_failed mmm,
-- aaa < mmm < zzz) apply purely from delivery order. The fix keeps the recorded
-- watermark when it is greater than the confirmation key: the started event
-- still wins the semantic tie and becomes active, but the pending event's
-- recorded key remains the ordering watermark, so mmm is stale while ordinary
-- chronological ordering over distinct events is unchanged.
...
declare
  ...
  v_confirmation_tie boolean := false;
...
begin
  ...
  -- applied event's own key. A same-stream confirmation that wins the
  -- equal-timestamp tie may order below the recorded pending key, so the
  -- override below keeps the recorded watermark instead of lowering it.
  ...
  if found then
    if v_state.source_system = v_event.source_system
      ...
      -- Semantic precedence must not rewind the reducer ordering watermark.
      -- When the confirmation's own key is below the recorded pending key the
      -- pending event stays the watermark, so a later same-timestamp event that
      -- sorts between them is stale instead of applying from delivery order.
