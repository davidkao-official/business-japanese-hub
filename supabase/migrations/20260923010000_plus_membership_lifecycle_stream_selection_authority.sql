-- #165 stream-selection repair: persist one immutable initial-start key per
-- source subscription and use it for cross-stream competition. Terminal clocks
-- remain scoped to their own stream. Historical ambiguity is never used to
-- infer a grant or selection key.
alter table public.plus_membership_subscription
  add column initial_start_occurred_at timestamptz,
  add column initial_start_event_id text references public.plus_membership_event(event_id),
  add constraint plus_membership_subscription_initial_start_pair check (
...
with unambiguous_start as (
  select source_system, source_customer_id, source_subscription_id,
         ...
         min(event_id) as initial_start_event_id
  from public.plus_membership_event
  where event_type = 'membership_started'
  group by source_system, source_customer_id, source_subscription_id
  having count(*) = 1
)
update public.plus_membership_subscription subscription
set initial_start_occurred_at = start.initial_start_occurred_at,
    initial_start_event_id = start.initial_start_event_id
...
comment on column public.plus_membership_subscription.initial_start_event_id is
  'Event identity paired with initial_start_occurred_at; conflicting later starts on this source stream fail closed.';
