-- #164 successor repair: retire the superseded stream when a newer start
-- selects a replacement stream, so a delayed event on the replaced stream can
-- never become current again.
--
-- The 14:00 terminal-authority writer remains intact. This migration only
-- replaces the writer function so that, before the reducer switches to a newer
-- noncurrent stream, the outgoing current stream's binding is retired durably.
-- Later evidence on the replaced stream then resolves through the existing
-- retired-binding path and cannot update state or access. All previously
-- established behaviors are preserved:
...
--   * terminal evidence for a replaced/noncurrent stream only retires that old
--     binding and never touches the current stream's access;
