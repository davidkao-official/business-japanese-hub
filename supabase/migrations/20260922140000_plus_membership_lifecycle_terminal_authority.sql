-- #164 successor repair: enforce terminal authority for ordering-stale terminal
-- evidence on the current stream, and make lifecycle audit event IDs
-- unambiguous for arbitrary nonempty source identifiers.
--
-- The 13:00 access-clamp writer remains intact. This migration only replaces
-- the writer function so that:
