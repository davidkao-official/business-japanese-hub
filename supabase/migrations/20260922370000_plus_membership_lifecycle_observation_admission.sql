-- #165: distinguish observed pending evidence from applied ordering; retain
-- trusted catalog eligibility at receipt for delayed reconciliation.
-- Historical event facts are not rewritten and unknown provenance fails closed.
alter table public.plus_membership_event
  add column plan_active_when_observed boolean,
  add column activation_eligible_when_observed boolean;
comment on column public.plus_membership_event.plan_active_when_observed is
  'Server-observed catalog activity at first receipt. Immutable; caller metadata cannot supply this fact.';
comment on column public.plus_membership_event.activation_eligible_when_observed is
  'Trusted receipt eligibility from active catalog, exact admission, or earlier eligible same-stream/plan evidence on an unadmitted stream. NULL historical provenance cannot authorize admission.';

alter table public.plus_membership_subscription
  add column predecessor_barrier_occurred_at timestamptz,
  add column predecessor_barrier_event_id text references public.plus_membership_event(event_id),
  add constraint plus_membership_subscription_predecessor_pair check (
    (predecessor_barrier_occurred_at is null) = (predecessor_barrier_event_id is null)
  );
create index plus_membership_subscription_predecessor_event_idx
  on public.plus_membership_subscription (predecessor_barrier_event_id);
-- A provisional selection retains inherited terminal authority even after
-- another confirmed stream displaces it from the per-user state row.
update public.plus_membership_subscription bound
set predecessor_barrier_occurred_at = state.succession_barrier_occurred_at,
    predecessor_barrier_event_id = state.succession_barrier_event_id
from public.plus_membership_state state
where bound.source_system = state.source_system
  and bound.source_customer_id = state.source_customer_id
  and bound.source_subscription_id = state.source_subscription_id
  and bound.user_id = state.user_id
  and bound.admitted_at is null
  and state.succession_barrier_occurred_at is not null;

alter table public.plus_membership_state
  add column applied_event_occurred_at timestamptz,
  add column applied_event_id text references public.plus_membership_event(event_id);

-- Recover only an exact current projection fact. Unknown legacy shapes retain
-- their former barrier conservatively; this migration never grants access.
update public.plus_membership_state state
set (applied_event_occurred_at, applied_event_id) = (
  select evidence.occurred_at, evidence.event_id
  from public.plus_membership_event evidence
  where evidence.source_system = state.source_system
    and evidence.source_customer_id = state.source_customer_id
    and evidence.source_subscription_id = state.source_subscription_id
    and evidence.user_id = state.user_id
    and evidence.plan_code = state.plan_code
    and evidence.period_start = state.period_start
    and evidence.period_end = state.period_end
    and evidence.cancel_at_period_end = state.cancel_at_period_end
    and (evidence.membership_status = state.membership_status
      or (state.membership_status = 'pending'
          and evidence.event_type = 'membership_canceled'
          and evidence.cancel_at_period_end))
    and (evidence.occurred_at, evidence.event_id)
        <= (state.last_event_occurred_at, state.last_event_id)
  order by evidence.occurred_at desc, evidence.event_id desc
  limit 1
);
update public.plus_membership_state
set applied_event_occurred_at = last_event_occurred_at,
    applied_event_id = last_event_id
where applied_event_id is null;
alter table public.plus_membership_state
  alter column applied_event_occurred_at set not null,
  alter column applied_event_id set not null;
create index plus_membership_state_applied_event_idx
  on public.plus_membership_state (applied_event_id);
comment on column public.plus_membership_state.applied_event_occurred_at is
  'Applied projection ordering key; ignored pending observations never advance it.';
comment on column public.plus_membership_state.last_event_occurred_at is
  'Observed current-stream watermark, including status-stale pending; not an access ordering barrier.';
...
-- by that exception, so the start may select/fold the retired stream.
  ...
  if found then
    ...
    if v_state.source_system = v_event.source_system
      ...
      v_ordering_barrier_occurred_at := v_state.applied_event_occurred_at;
      v_ordering_barrier_event_id := v_state.applied_event_id;
