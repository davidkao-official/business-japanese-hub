-- #164 / #165: terminal reconciliation and cancellation admission repair.
-- Confirmation may outrank pending time, but never durable terminal evidence.
-- A scheduled cancellation records a cutoff without confirming an unpaid stream.
-- Function-only append: preserve event facts, schema, grants and migration history.
...
-- by that exception, so the start may select/fold the retired stream.
  ...
  -- the projection instead of promoting access. A same-stream pending
  -- confirmation also folds durable terminal evidence even below its monotonic
  -- watermark: semantic precedence over pending can never override revocation
  -- or a scheduled cutoff. Folding that evidence does not rewind the marker.
  if v_stream_became_current or v_confirming_pending then
    ...
    where buffered.source_system = v_event.source_system
      ...
      and (
        ...
        or (v_confirming_pending and (
          buffered.event_type in (
            'membership_expired', 'membership_revoked', 'membership_refunded',
            'membership_reversed', 'membership_disputed', 'membership_canceled'
          )
        ))
      )
    order by (
