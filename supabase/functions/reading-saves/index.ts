/** Member-owned Business Reading save API; bearer and Plus access are reverified in handler. */
...
import { handleReadingSaves, MAX_BODY_BYTES } from './handler.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return toResponse(jsonResult(400, { error: 'invalid Reading save request' }))
  }
  ...
  try {
    ...
    return toResponse(withCorsHeaders(jsonResult(503, { error: 'Reading saves unavailable' }), cors.headers))
  }
  const result = await handleReadingSaves(request, {
    db,
