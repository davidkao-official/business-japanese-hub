...
Deno.serve(async (req) => {
  ...
  // Safe provider registry: an ECPay-only deployment boots (PayPal becomes a
  // fail-closed stub); the handler refuses USD with a provider-configuration
  // 422 before creating any Order/Payment row (§21).
  const adapters = createProviderAdapters(env);
  const log = createSanitizedLogger();
  const deps: CheckoutHandlerDeps = { env, db, adapters, log };

  request.bodyText = await req.text();
  return toResponse(withCorsHeaders(await handleCheckout(request, deps), cors.headers));
});
