...
-- Prefix ending just before original assertion 32 (the bigint q-large record call).
begin;
select plan(32);
select todo('diagnostic prefix probe: superseded original assertions 1..31', 31);
...
select pass('diagnostic prefix probe: reached just before original assertion 32');
select * from finish();
rollback;
