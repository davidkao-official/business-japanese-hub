...
select is((select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-641'),(select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-640'),'failure delivery orders converge on C selection time');
select is((select array_agg(event.source_event_id order by subscription.source_subscription_id) from public.plus_membership_subscription subscription join public.plus_membership_event event on event.event_id=subscription.initial_start_event_id where subscription.source_subscription_id in ('C-640','C-641')),array['start-C-640','start-C-641']::text[],'failure delivery orders retain each C key source event');
...
select is((select split_part(source_subscription_id,'-',1) from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000657'),(select split_part(source_subscription_id,'-',1) from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000642'),'renewal delivery orders select the same final stream role');
...
select is((select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-657'),(select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-642'),'renewal delivery orders converge on C selection time');
select is((select array_agg(event.source_event_id order by subscription.source_subscription_id) from public.plus_membership_subscription subscription join public.plus_membership_event event on event.event_id=subscription.initial_start_event_id where subscription.source_subscription_id in ('C-642','C-657')),array['start-C-642','start-C-657']::text[],'renewal delivery orders retain each C key source event');
...
select is((select split_part(source_subscription_id,'-',1) from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000652'),(select split_part(source_subscription_id,'-',1) from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000643'),'immediate terminal delivery orders select the same final stream role');
...
select is((select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-652'),(select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-643'),'immediate terminal delivery orders converge on C selection time');
select is((select array_agg(event.source_event_id order by subscription.source_subscription_id) from public.plus_membership_subscription subscription join public.plus_membership_event event on event.event_id=subscription.initial_start_event_id where subscription.source_subscription_id in ('C-643','C-652')),array['start-C-643','start-C-652']::text[],'immediate terminal delivery orders retain each C key source event');
...
select is((select split_part(source_subscription_id,'-',1) from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000653'),(select split_part(source_subscription_id,'-',1) from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000644'),'scheduled cutoff delivery orders select the same final stream role');
...
select is((select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-653'),(select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-644'),'scheduled cutoff delivery orders converge on C selection time');
select is((select array_agg(event.source_event_id order by subscription.source_subscription_id) from public.plus_membership_subscription subscription join public.plus_membership_event event on event.event_id=subscription.initial_start_event_id where subscription.source_subscription_id in ('C-644','C-653')),array['start-C-644','start-C-653']::text[],'scheduled cutoff delivery orders retain each C key source event');
...
select is((select split_part(source_subscription_id,'-',1) from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000654'),(select split_part(source_subscription_id,'-',1) from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000646'),'terminal-first confirmation orders select the same final stream role');
...
select is((select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-654'),(select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-646'),'terminal-first confirmation orders converge on C selection time');
select is((select array_agg(event.source_event_id order by subscription.source_subscription_id) from public.plus_membership_subscription subscription join public.plus_membership_event event on event.event_id=subscription.initial_start_event_id where subscription.source_subscription_id in ('C-646','C-654')),array['start-C-646','start-C-654']::text[],'terminal-first confirmation orders retain each C key source event');
...
select is((select split_part(source_subscription_id,'-',1) from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000655'),(select split_part(source_subscription_id,'-',1) from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000645'),'provisional delivery orders select the same final stream role');
...
select is((select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-655'),(select initial_start_occurred_at from public.plus_membership_subscription where source_subscription_id='C-645'),'provisional delivery orders converge on C selection time');
select is((select array_agg(event.source_event_id order by subscription.source_subscription_id) from public.plus_membership_subscription subscription join public.plus_membership_event event on event.event_id=subscription.initial_start_event_id where subscription.source_subscription_id in ('C-645','C-655')),array['start-C-645','start-C-655']::text[],'provisional delivery orders retain each C key source event');
