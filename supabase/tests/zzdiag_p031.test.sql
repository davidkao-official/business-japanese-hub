...
-- Prefix of original statements ending just before original assertion 31 (server persistence stores bounded attempts).
-- All 30 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(31);
select todo('diagnostic prefix probe: superseded original assertions 1..30', 30);
...
select pass('diagnostic prefix probe: reached just before original assertion 31');
select * from finish();
rollback;
