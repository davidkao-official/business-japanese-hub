...
-- Prefix of original statements ending just before original assertion 33 (persisted question version retains 2147483648).
-- All 32 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(33);
select todo('diagnostic prefix probe: superseded original assertions 1..32', 32);
...
select pass('diagnostic prefix probe: reached just before original assertion 33');
select * from finish();
rollback;
