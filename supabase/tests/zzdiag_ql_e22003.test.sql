...
-- Does the q-large record call raise SQLSTATE 22003 (numeric out of range)?
begin;
select plan(3);
...
select throws_ok($$ select public.record_practice_attempt('61000000-0000-0000-0000-000000000001','61100000-0000-4000-8000-000000000005','practice-web-test-spi-v1',repeat('a',64),'q-large',2147483648,'spi','verbal','vocabulary-in-context','untimed-learning','{"input":"two"}'::jsonb,true,1000,'[]'::jsonb) $$, '22003', null, 'probe: q-large raises 22003');
select * from finish();
rollback;
