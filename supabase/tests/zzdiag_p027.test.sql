...
-- Prefix of original statements ending just before original assertion 27 (atomic import stores the immutable release).
-- All 26 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(27);
select todo('diagnostic prefix probe: superseded original assertions 1..26', 26);
...
select pass('diagnostic prefix probe: reached just before original assertion 27');
select * from finish();
rollback;
