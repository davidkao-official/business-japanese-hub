...
-- Prefix of original statements ending just before original assertion 34 (question version regression aborts and rolls back its releas).
-- All 33 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(34);
select todo('diagnostic prefix probe: superseded original assertions 1..33', 33);
...
select pass('diagnostic prefix probe: reached just before original assertion 34');
select * from finish();
rollback;
