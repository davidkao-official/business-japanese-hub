...
select is(public.record_plus_membership_event('source-provisional','customer-706','subscription-706-b','start-706-b30','50000000-0000-0000-0000-000000000706','plus_early_access_monthly','membership_reactivated',now() - interval '1 day',now() - interval '1 day',now() + interval '30 days'),'stale','#165 confirmed predecessor 706 does not resurrect retired B at t30');
...
select is((select predecessor_barrier_occurred_at from public.plus_membership_subscription where source_subscription_id='subscription-707-b'),now() - interval '30 days','#165 displaced provisional pending preserves predecessor initial-start time');
select is((select predecessor_barrier_event_id from public.plus_membership_subscription where source_subscription_id='subscription-707-b'),(select event_id from public.plus_membership_event where source_system='source-provisional' and source_event_id='start-707-a'),'#165 displaced provisional pending preserves predecessor initial-start identity');
select is(public.record_plus_membership_event('source-provisional','customer-707','subscription-707-b','pending-707-b4','50000000-0000-0000-0000-000000000707','plus_early_access_monthly','membership_pending',now() - interval '26 days',now() - interval '26 days',now() + interval '24 days'),'stale','#165 inherited barrier 707 displaced pending evidence creates no start authority');
select is(public.record_plus_membership_event('source-provisional','customer-707','subscription-707-b','start-707-b30','50000000-0000-0000-0000-000000000707','plus_early_access_monthly','membership_started',now() - interval '1 day',now() - interval '1 day',now() + interval '30 days'),'applied','#165 inherited barrier 707 B initial start after C becomes current');
