...
-- Provisional B inherits A's initial-start key at t0, not the later terminal
-- at t5. B's t4 confirmation is therefore valid; later changes use reactivation.
...
select is((select succession_barrier_occurred_at from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000216'),(select occurred_at from public.plus_membership_event where source_event_id='start-216-a'),'#165 barrier stores predecessor terminal time');
select is((select succession_barrier_event_id from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000216'),(select event_id from public.plus_membership_event where source_system='source-a' and source_event_id='start-216-a'),'#165 barrier stores predecessor terminal identity');
...
select is((select succession_barrier_occurred_at from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000216'),(select occurred_at from public.plus_membership_event where source_event_id='start-216-a'),'#165 barrier repeated pending preserves predecessor initial-start time');
select is((select succession_barrier_event_id from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000216'),(select event_id from public.plus_membership_event where source_system='source-a' and source_event_id='start-216-a'),'#165 barrier repeated pending preserves predecessor initial-start identity');
...
select is(public.record_plus_membership_event('source-a','customer-216','subscription-216-b','start-216-b-old','50000000-0000-0000-0000-000000000216','plus_early_access_monthly','membership_started',now() - interval '21 days',now() - interval '21 days',now() + interval '15 days'),'applied','#165 initial-start authority accepts B confirmation at t4');
select is((select source_subscription_id from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000216'),'subscription-216-b','#165 accepted t4 confirmation keeps B current');
select is((select membership_status from public.plus_membership_state where user_id='50000000-0000-0000-0000-000000000216'),'active','#165 accepted t4 confirmation leaves B active');
select is((select membership_status from public.plus_membership_access where user_id='50000000-0000-0000-0000-000000000216'),'active','#165 accepted t4 confirmation grants access');
select is(public.record_plus_membership_event('source-a','customer-216','subscription-216-b','start-216-b','50000000-0000-0000-0000-000000000216','plus_early_access_monthly','membership_reactivated',now() - interval '15 days',now() - interval '15 days',now() + interval '15 days'),'applied','#165 barrier accepts B confirmation at t10');
