...
-- Prefix of original statements ending just before original assertion 62 (latest unsupported delivery profile is non-actionable).
-- All 61 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(62);
select todo('diagnostic prefix probe: superseded original assertions 1..61', 61);
...
select pass('diagnostic prefix probe: reached just before original assertion 62');
select * from finish();
rollback;
