...
-- Prefix of original statements ending just before original assertion 52 (new release attempt is persisted).
-- All 51 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(52);
select todo('diagnostic prefix probe: superseded original assertions 1..51', 51);
...
select pass('diagnostic prefix probe: reached just before original assertion 52');
select * from finish();
rollback;
