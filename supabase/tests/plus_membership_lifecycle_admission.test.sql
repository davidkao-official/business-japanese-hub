...
select is(public.record_plus_membership_event('source-a','customer-202','subscription-202','pending-202','50000000-0000-0000-0000-000000000202','plus_early_access_monthly','membership_pending',(now() - interval '21 days'),(now() - interval '21 days'),(now() + interval '9 days')),'applied','#164 inactive-plan admission records an unconfirmed pending binding');
...
select throws_ok($$select public.record_plus_membership_event('source-a','customer-202','subscription-202','start-202','50000000-0000-0000-0000-000000000202','plus_early_access_monthly','membership_started',(now() - interval '20 days'),(now() - interval '20 days'),(now() + interval '10 days'))$$, 'P0001', 'plan plus_early_access_monthly is not active for membership activation', '#164 inactive-plan admission rejects a start on a pending-created binding');
...
select is(public.record_plus_membership_event('source-a','customer-203','subscription-203','start-203','50000000-0000-0000-0000-000000000203','plus_early_access_monthly','membership_started',(now() - interval '21 days'),(now() - interval '21 days'),(now() + interval '40 days')),'applied','#164 inactive-plan admission starts and admits a valid binding');
...
update public.plus_membership_plan set active = false where plan_code = 'plus_early_access_monthly';
select is(public.record_plus_membership_event('source-a','customer-203','subscription-203','cancel-203','50000000-0000-0000-0000-000000000203','plus_early_access_monthly','membership_canceled',(now() - interval '20 days'),(now() - interval '21 days'),(now() + interval '40 days'),true),'applied','#164 inactive-plan admission still accepts period-end cancellation on an admitted binding');
select is((select terminal_at from public.plus_membership_subscription where source_subscription_id='subscription-203'),(now() + interval '40 days'),'#164 inactive-plan admission records the admitted stream cutoff');
...
select is((select current_period_end from public.plus_membership_access where user_id='50000000-0000-0000-0000-000000000203'),(now() + interval '40 days'),'#164 inactive-plan admission clamps the admitted stream at the recorded cutoff');
select is(public.record_plus_membership_event('source-a','customer-203','subscription-203','renew-203','50000000-0000-0000-0000-000000000203','plus_early_access_monthly','membership_renewed',(now() - interval '7 days'),(now() + interval '40 days'),(now() + interval '70 days')),'applied','#164 inactive-plan admission accepts a pre-cutoff renewal on an admitted binding');
select is((select current_period_end from public.plus_membership_access where user_id='50000000-0000-0000-0000-000000000203'),(now() + interval '40 days'),'#164 inactive-plan admission keeps the admitted stream clamped at the cutoff');
