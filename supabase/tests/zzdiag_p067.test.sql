...
-- Prefix of original statements ending just before original assertion 67 (all-ineligible release leaves no availability rows).
-- All 66 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(67);
select todo('diagnostic prefix probe: superseded original assertions 1..66', 66);
...
select pass('diagnostic prefix probe: reached just before original assertion 67');
select * from finish();
rollback;
