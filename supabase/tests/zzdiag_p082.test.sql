...
-- Prefix of original statements ending just before original assertion 82 (browser cannot call service persistence).
-- All 81 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(82);
select todo('diagnostic prefix probe: superseded original assertions 1..81', 81);
...
select pass('diagnostic prefix probe: reached just before original assertion 82');
select * from finish();
rollback;
