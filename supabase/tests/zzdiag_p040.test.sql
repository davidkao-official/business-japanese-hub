...
-- Prefix of original statements ending just before original assertion 40 (mismatched idempotency key conflicts).
-- All 39 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(40);
select todo('diagnostic prefix probe: superseded original assertions 1..39', 39);
...
select pass('diagnostic prefix probe: reached just before original assertion 40');
select * from finish();
rollback;
