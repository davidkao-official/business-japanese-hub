...
-- Prefix of original statements ending just before original assertion 41 (client retry is idempotent alongside the bigint-version atte).
-- All 40 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(41);
select todo('diagnostic prefix probe: superseded original assertions 1..40', 40);
...
select pass('diagnostic prefix probe: reached just before original assertion 41');
select * from finish();
rollback;
