...
-- Prefix of original statements ending just before original assertion 56 (same release availability retry is idempotent).
-- All 55 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(56);
select todo('diagnostic prefix probe: superseded original assertions 1..55', 55);
...
select pass('diagnostic prefix probe: reached just before original assertion 56');
select * from finish();
rollback;
