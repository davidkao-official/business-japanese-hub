...
-- Prefix of original statements ending just before original assertion 39 (identical retry returns a replayable conflict).
-- All 38 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(39);
select todo('diagnostic prefix probe: superseded original assertions 1..38', 38);
...
select pass('diagnostic prefix probe: reached just before original assertion 39');
select * from finish();
rollback;
