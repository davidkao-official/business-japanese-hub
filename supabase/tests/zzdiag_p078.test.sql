...
-- Prefix of original statements ending just before original assertion 78 (retired q1 and q2 leave no authenticated review queue).
-- All 77 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(78);
select todo('diagnostic prefix probe: superseded original assertions 1..77', 77);
...
select set_config('request.jwt.claims','{"role":"authenticated"}',true);
select set_config('request.jwt.claim.sub', '', true);
select pass('diagnostic prefix probe: reached just before original assertion 78');
select * from finish();
rollback;
