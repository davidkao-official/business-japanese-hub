...
-- Prefix of original statements ending just before original assertion 74 (supported practice mode becomes actionable again).
-- All 73 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(74);
select todo('diagnostic prefix probe: superseded original assertions 1..73', 73);
...
select pass('diagnostic prefix probe: reached just before original assertion 74');
select * from finish();
rollback;
