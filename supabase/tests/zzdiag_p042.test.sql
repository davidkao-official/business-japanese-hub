...
-- Prefix of original statements ending just before original assertion 42 (later correct clears the exact question review).
-- All 41 prefix assertions are TODO-suppressed with validated correct usage;
...
begin;
select plan(42);
select todo('diagnostic prefix probe: superseded original assertions 1..41', 41);
...
select pass('diagnostic prefix probe: reached just before original assertion 42');
select * from finish();
rollback;
